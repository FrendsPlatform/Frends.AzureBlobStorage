...
public static class AzureBlobStorage
{
    ...
    public static async Task<Result> ListBlobsInContainer([PropertyTab] Source source, [PropertyTab] Connection connection, [PropertyTab] Options options,
        CancellationToken cancellationToken)
    {
        ValidationHandler.Run(source, connection);
        return new Result(await ListBlobHandler(source, connection, options, cancellationToken));
    }

    private static async Task<List<BlobData>> ListBlobHandler(Source source, Connection connection, Options options,
        CancellationToken cancellationToken)
    {
        var blobContainerClient = ConnectionHandler.GetBlobContainerClient(connection, source.ContainerName, cancellationToken);
