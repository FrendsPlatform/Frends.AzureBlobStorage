...
public static class AzureBlobStorage
{
    ...
    /// <returns>Object { bool Success, List&lt;BlobData&gt; BlobList [ { string Type, string Name, string URL, string ETag, DateTimeOffset? CreatedOn, DateTimeOffset? LastModified } ], Error { string Message, Exception AdditionalInfo } }</returns>
    public static async Task<Result> ListBlobsInContainer([PropertyTab] Input input, [PropertyTab] Connection connection, [PropertyTab] Options options,
        CancellationToken cancellationToken)
    {
        try
        {
            ValidationHandler.Run(input, connection);
            return new Result { Success = true, BlobList = await ListBlobHandler(input, connection, options, cancellationToken), Error = null };
        }
        catch (Exception ex)
        {
    ...
    private static async Task<List<BlobData>> ListBlobHandler(Input input, Connection connection, Options options,
        CancellationToken cancellationToken)
    {
        var blobContainerClient = ConnectionHandler.GetBlobContainerClient(connection, input.ContainerName, cancellationToken);
