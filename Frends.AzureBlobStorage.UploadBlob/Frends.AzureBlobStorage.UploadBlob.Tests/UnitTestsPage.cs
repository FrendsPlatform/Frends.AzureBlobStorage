...
public class PageTests
{
    ...
    [TestMethod]
    public async Task UploadFile_CalculatePageSizeBlob()
    {
        ...
        // connection string
        var result3 = await AzureBlobStorage.UploadBlob(input, destinationCS, options, default);
        Assert.IsTrue(result3.Success);
        Assert.IsTrue(result3.Data.ContainsValue($"{container.Uri}/testfile.txt"));
        Assert.IsTrue(await container.GetBlobClient("testfile.txt").ExistsAsync(), "Uploaded testfile.txt blob should exist");

        // OAuth
        var result4 = await AzureBlobStorage.UploadBlob(input, destinationOA, options, default);
        Assert.IsTrue(result4.Success);
        Assert.IsTrue(result4.Data.ContainsValue($"{container.Uri}/testfile.txt"));
        Assert.IsTrue(await container.GetBlobClient("testfile.txt").ExistsAsync(), "Uploaded testfile.txt blob should exist");
    }

    [TestMethod]
    public async Task UploadFile_OffsetToEnd()
    {
        var options = new Options() { ThrowErrorOnFailure = true };
    ...
    // Note: Currently tags haven't been supported but they shouldn't cause an error neither.
    [TestMethod]
    ...
    public async Task UploadFile_HandleExistingFile()
    {
        ...
        foreach (var handler in errorHandlers)
        {
            ...
            {
                var result2 = await AzureBlobStorage.UploadBlob(input2, destination, options, default);
                // Ýou can use Azure Portal to check if the blob contains "More text" and empty bytes.
                Assert.IsTrue(result2.Success);
                Assert.IsTrue(result2.Data.ContainsValue($"{container.Uri}/testfile.txt"));
                Assert.IsTrue(await container.GetBlobClient("testfile.txt").ExistsAsync(), "Uploaded testfile.txt blob should exist");
            }
            ...
            else
            {
                var result2 = await AzureBlobStorage.UploadBlob(input, destination, options, default);
                Assert.IsTrue(result2.Success);
                Assert.IsTrue(result2.Data.ContainsValue($"Blob testfile.txt already exists."));

                var ex = await Assert.ThrowsExceptionAsync<Exception>(() => AzureBlobStorage.UploadBlob(input, destination, new Options { ThrowErrorOnFailure = true }, default));
    ...
    [TestMethod]
    public async Task UploadDirectory_2Files()
    {
        var tags = new Tag[] { new Tag { Name = "TagName", Value = "TagValue" } };
        var options = new Options() { ThrowErrorOnFailure = true };
        ...
        // connection string
        var resultWithTags = await AzureBlobStorage.UploadBlob(inputWithTags, destination, options, default);
        Assert.IsTrue(resultWithTags.Success);
    ...
    [TestMethod]
    public async Task UploadDirectory_2Files_RenameDir()
    {
        var tags = new Tag[] { new Tag { Name = "TagName", Value = "TagValue" } };
        var options = new Options() { ThrowErrorOnFailure = true };
        ...
        var resultWithTags = await AzureBlobStorage.UploadBlob(inputWithTags, destination, options, default);
        Assert.IsTrue(resultWithTags.Success);
        Assert.IsTrue(resultWithTags.Data.ContainsValue($"{container.Uri}/Renamed/testfile.txt"));
        Assert.IsTrue(await container.GetBlobClient("Renamed/testfile.txt").ExistsAsync(), "Uploaded testfile.txt blob should exist");
        Assert.IsTrue(resultWithTags.Data.ContainsValue($"{container.Uri}/Renamed/testfile2.txt"));
        Assert.IsTrue(await container.GetBlobClient("Renamed/testfile2.txt").ExistsAsync(), "Uploaded testfile2.txt blob should exist");
    }
    ...
    private void CreateFiles()
    {
        ...
        File.WriteAllText(_testfile2, "More text.");
    }
