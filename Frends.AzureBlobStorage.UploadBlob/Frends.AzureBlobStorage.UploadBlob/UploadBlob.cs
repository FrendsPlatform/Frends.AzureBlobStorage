...
public class AzureBlobStorage
{
		...
		switch (options.BlobType)
		{
			case AzureBlobType.Append:
				try
				{
					...
					//	ConnectionHandler.GetBlobClient(connection, options, blobName,
					//		cancellationToken);
				...
				try
				{
					...
					if (fi != null)
					{
						...
						if (File.Exists(fi.FullName) &&
							...
							!fi.FullName.StartsWith(input.SourceDirectory ?? string.Empty, StringComparison.OrdinalIgnoreCase))
							fi.Delete();
					}
			...
			case AzureBlobType.Page:
				try
				{
					...
					if (options.ResizeFile && bytesMissing > 0)
					{
						...
						if (exists)
							await pageBlobClient.ResizeAsync(fiMinLenght + bytesMissing,
					...
					await pageBlobClient.SetHttpHeadersAsync(new BlobHttpHeaders
					{
						ContentType = contentType,
						ContentEncoding = input.Compress ? "gzip" : encoding.WebName
					}, cancellationToken: cancellationToken);
					...
					if (tags.Count > 0)
						await pageBlobClient.SetTagsAsync(tags, cancellationToken: cancellationToken);
	...
	private static async Task CreateContainerIfItDoesNotExist(Connection connection, string containerName,
		CancellationToken cancellationToken)
	{
		try
		{
			...
			else if (connection.AuthenticationMethod is AuthenticationMethod.SASToken)
			{
	...
	private static Stream GetReencodedStream(FileInfo file, Encoding encoding)
	{
		...
		try
		{
			...
			using (var reader = new StreamReader(source, CreateStrictEncoding(encoding),
					   detectEncodingFromByteOrderMarks: true, bufferSize: StreamBufferSize, leaveOpen: false))
	...
	private static void CheckParameters(Input input, Connection connection)
	{
		...
		if (connection.AuthenticationMethod is AuthenticationMethod.OAuth2 && (string.IsNullOrEmpty(connection.ApplicationId) ||
